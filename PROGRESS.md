...
## Next action
Continue BACKLOG.md P0 in the owner's verdict order: **P0-21 (income preview, gold panel & round-end summary)** next, then P0-22…P0-24 (round track, shop HUD, augments). P0-B4…P0-B12 stay deferred/queued per the triage rule (P0-B11/P0-B12 are money-path bugs QA filed on P0-20, ranked above the rest of section 3 but still below the remaining owner-request items).

## Pipeline checks
- `npm run check` — green (tsc + architecture test, 99 tests).
- `npm run test:fast` — green, 349 tests + 1 documented skip in 24 files; measured wall time **~3.3 s** (limit 5 min).
