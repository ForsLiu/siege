...
## Next action
Continue BACKLOG.md P0 in the owner's verdict order: **P0-23 (shop HUD)** next, then P0-24 (augments). P0-B4…P0-B15 stay deferred/queued per the triage rule (P0-B11/P0-B12 are money-path bugs QA filed on P0-20, ranked above the rest of section 3 but still below the remaining owner-request items; P0-B13/14/15 are P0-21 polish deferrals; P0-B13's own branch is still unreached, but the sibling issue P0-21-03 flagged — `summaryLines`' "next round" line stood in with the encounter id — was fixed in this same P0-22 pass now that `type` exists).

## Pipeline checks
- `npm run check` — green (tsc + architecture test, 99 tests).
- `npm run test:fast` — green, 356 tests + 1 documented skip in 24 files; measured wall time **~3.4 s** (limit 5 min).
