...
Environment: `SIEGE_TEST_WORKERS` (vitest workers, default 4) and `SIEGE_SWEEP_WORKERS` (sweep worker threads, default 4) are separate knobs — the sweep never reads `SIEGE_TEST_WORKERS`. Both take an integer in 1..64, are trimmed, and treat an empty value as unset; anything else fails loudly instead of falling back (an invalid `SIEGE_TEST_WORKERS` used to make vitest collect nothing and still exit 0, and `SIEGE_SWEEP_WORKERS` used to bypass the range the `--workers` flag enforces).
