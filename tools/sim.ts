...
runTool(() => {
  ...
  for (const r of report.rounds) {
    console.log(`${String(r.round).padStart(5)}  ${String(r.hpAfter).padStart(3)}  ${String(r.gold).padStart(4)} ${String(r.level).padStart(3)}  ${r.fight.winner.padEnd(5)} -${String(r.fight.hpLoss).padStart(2)}  ${String(r.fight.ticks).padStart(5)}  ${r.board.join(' ')}`);
  }
